...
288. **[done]** Round-5 verification: full local gate green (generalized validators, `actionlint` incl. `scorecard.yml`, `ruff`, 27 tests, link/catalog/settings checks, hook smoke). The independent QA stand-in **caught a runtime bug `actionlint` can't see** — Scorecard v5.x gates `--format=sarif` behind `ENABLE_SARIF=1` (added) — by exercising the pinned binary directly. The full scan validates on the Actions runner post-merge (`api.github.com`/`api.osv.dev` are proxy-blocked in-sandbox).
